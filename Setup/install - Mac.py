...
SERVICE_NAME = "system_stats"  # used for plist/service naming on macOS
...
def download_file(url, path):
    ...
    try:
        ...
        print(f"urllib failed: {e}. Trying curl/wget...")
...
def add_to_startup(os_name, script_path):
    if os_name == "Windows":
        ...
        plist = os.path.join(plist_dir, f"com.{SERVICE_NAME}.startup.plist")
        plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.{SERVICE_NAME}.startup</string>
...
"""
        ...
        # try to load using launchctl (user domain)
        try:
            subprocess.run(["launchctl", "load", "-w", plist], check=True)
            print("Added launchd plist and loaded it.")
            return
        except Exception as e:
            print(f"Failed to load plist via launchctl: {e}. The plist was written to {plist}")
